...
#'
#' @importFrom stats aggregate as.formula model.matrix
...
predict_lavaan <- function(fit, newdata = NULL,
  ...
  if(!lavInspect(fit, "options")$meanstructure) stop("Need to supply meanstructure = TRUE in fit\n")
  if(is.null(newdata)){
    newdata <- data.frame(lavInspect(fit, "data"))
...
residuals_lavaan <- function(fit){
  ...
  rawdata <- data.frame(lavInspect(fit, "data"))
...
get_sim_ci_lavaan <- function(fit, mod_df, newdata, nsims, ci, interval="confidence"){
  ...
  if (!("package:mvtnorm" %in% search())) attachNamespace("mvtnorm")
  ...
  lower_ci <- aggregate(. ~ pt, data=sims, quantile, probs=(1-ci)/2)[,-1]
  names(lower_ci) <- paste0(names(sims)[-ncol(sims)], "_lower")

  upper_ci <- aggregate(. ~ pt, data=sims, quantile, probs=1-(1-ci)/2)[,-1]
